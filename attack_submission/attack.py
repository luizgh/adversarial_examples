...
#        return model_dense.predict(img) + model_v3.predict(img) + model_v3_adv.predict(img) + model_v4.predict(img) + model_incept_resnet_v2.predict(img) + model_incept_resnet_adv_ens.predict(img)
    ...
    with tf.Session(config=config) as sess:
      ...
      adversarial_generator.initialize(sess)

      batch_num = 0
      for filenames, images in image_loader:
        predicted_class = sess.run(pred, feed_dict={x_input:images})
        adv_images = adversarial_generator.generate(sess, images, predicted_class, verbose=True)
