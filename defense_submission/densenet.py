  ...
  with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      net = slim.conv2d(net, num_classes, 1,
                        ...
                        scope='logits')
      net = tf.concat([tf.zeros([16,1,1,1], tf.float32), net], 3)
      end_points = slim.utils.convert_collection_to_dict(
          end_points_collection)

      if num_classes is not None:
        end_points['Predictions'] = slim.softmax(net, scope='predictions')[:,0,0]
        end_points['Logits'] = net[:,0,0]
...
def densenet169(inputs, num_classes=1000, is_training=True, reuse=None,scope='densenet169'):
  return densenet(inputs,
                  num_classes=num_classes, 
                  reduction=0.5,
                  growth_rate=32,
